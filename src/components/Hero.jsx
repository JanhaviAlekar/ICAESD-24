...
export const Hero = () => {
  ...
  return (
    <div className="relative bg-gradient-to-r from-purple-600 to-blue-600 h-full text-white overflow-hidden">
      ...
      {/* 🔥 Sliding Banner at Top */}
      <div className="absolute top-0 w-full overflow-hidden text-black font-medium px-8 py-1 sm:py-2 z-20 mb-3">
        <a href="#/register">
          <div className="flex whitespace-nowrap animate-slide">
            <div className="flex items-center gap-2 py-1 sm:gap-3 px-2 sm:px-4">
              <img src={newImg} alt="New" className="w-5 h-5 sm:w-6 sm:h-6" />
              <span className="font-semibold text-sm sm:text-base bg-yellow-300 p-1.5 rounded-md">
                Review process will be updated by 25th October
              </span>
            </div>
            <div className="flex  items-center gap-2 sm:gap-3 px-2 sm:px-4">
              <img src={newImg} alt="New" className="w-5 h-5 sm:w-6 sm:h-6" />
              <span className="font-semibold text-sm sm:text-base bg-yellow-300 p-1.5 rounded-md">
                Camera Ready Copy Submission need to be done on
                camera-ready-icaiisd25@gcoea.ac.in & through CMT login
              </span>
            </div>
            <div className="flex  items-center gap-2 sm:gap-3 px-2 sm:px-4">
              <img src={newImg} alt="New" className="w-5 h-5 sm:w-6 sm:h-6" />
              <span className="font-semibold text-sm sm:text-base bg-yellow-300 p-1.5 rounded-md">
                SBI Collect Payment Process has been Started
              </span>
            </div>
          </div>
        </a>
      ...
      <div className="mt-4 relative z-10 flex flex-col justify-center items-center text-center px-2 pt-16 pb-10">
        {/* Date Update Banner */}
        ...
        <p className="lg:text-3 text-2xl font-bold mt-7 text-yellow-300 pb-2">
          2<sup>nd</sup> International Conference on Artificial Intelligence
          (AI) Innovations for Societal Development
        </p>
        ...
        <div className="flex justify-center gap-6 text-center bg-black bg-opacity-30 p-6 rounded-xl shadow-lg mt-4">
          {["Days", "Hours", "Minutes", "Seconds"].map((label, idx) => (
            <div key={label} className="flex flex-col items-center">
              ...
              <span className="uppercase text-sm lg:text-base text-gray-200 tracking-wide">
                {label}
              </span>
            </div>
          ))}
